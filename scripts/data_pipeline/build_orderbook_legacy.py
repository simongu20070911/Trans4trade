"""Legacy orderbook builder retained for reference only."""
...
for date in dates:
  if os.path.isfile(os.path.join('./data/input_data',date,'orderbook.csv')):
    print(f'Skipping files from {date} as they already exist.\n')
    continue
  
  print(f'Processing files from {date}.\n')
  
  input_path = './data/raw/'+date+'/orderbook*'
  output_path = os.path.join('./data/input_data', date)
  ...
  orderbook.to_csv(f'./data/input_data/{date}/orderbook.csv', index=False)

...
# S4 MODEL
################################################################################
# NOTE: Handling of dropout in different PyTorch versions
...
class S4Model(nn.Module):
    """
    S4Model: Stacked S4 layers as residual blocks + final linear decoder.
    """
    def __init__(
        self,
        d_input,
    ...
    def forward(self, x):
        ...
        for layer, norm, dropout in zip(self.s4_layers, self.norms, self.dropouts):
            ...
            # Apply S4 block (ignore state input/output)
            z, _ = layer(z)
        ...
        if torch.isnan(x).any():
            print("NaN detected before decoder")

        x = self.decoder(x)  # (B, L, d_model) -> (B, L, d_model)
        if torch.isnan(x).any():
            print("NaN detected after decoder")
        return x
...
class S4Transformer(nn.Module):
    """
    Combine S4 model with a Transformer Encoder.
    """
    def __init__(self, input_size, d_model=36, nhead=4, num_layers=3, output_size=1):
        super().__init__()
        self.s4 = S4Model(
            d_input=38,  # Hard-coded for demonstration
            d_output=1,
            d_model=36,
            n_layers=3,
            dropout=0.1,
    ...
    def forward(self, x):
        ...
        if torch.isnan(x).any():
            print("NaN detected after TransformerEncoder")

        x = x.permute(1, 0, 2)

        if torch.isnan(x).any():
            print("NaN detected before final mean")

        x = x.mean(dim=1)
        if torch.isnan(x).any():
            print("NaN detected after mean")
...
if __name__ == "__main__":
    ...
    base_save_path = os.path.join(
        ...
        model_name,
        "training_details/HFTransformer/results_HFformer"
    )
    ...
    for forecast_window in forecast_windows:
        ...
        split_index, data_x_train, data_y_train, data_x_val, data_y_val = prepare_data(
            np.array(orderbook[features][1_000_000:1_350_000]),
        ...
        final_model_path = os.path.join(
            "/home/gaen/Documents/codespace-gaen/Ts-master/playround_models/",
            model_name,
            f"transformer_enclinear_forecasting_FINAL_horizon_{forecast_window}.pt",
        )
